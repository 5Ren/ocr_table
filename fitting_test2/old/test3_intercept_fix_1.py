...
color_list = ['#050505', '#4472C4', '#ED7D31', '#A5A5A5', '#FFC000', '#5B9BD5', '#70AD47', '#7030A0']
sample_name = ['Water', 'CNF0.05wt%', '0.1wt%', '0.2wt%', '0.3wt%', '0.4wt%', 'Plasma']
marker_list = ['o', 'o', 'o', 'o', 'o', 'o', 'x']

# CSVファイルの読み込み
data = np.loadtxt('', delimiter=',', encoding='utf-8_sig')
...
print('=' * 40)
current_y_data = np.array(data[1, :])

# 新しいx軸の範囲を作成
new_x = np.linspace(min(x), max(x), 100)  # 100個の等間隔なx値を生成


intercept_y = current_y_data[-1]

def nonlinear_fit(x, a, b):
    # return -a ** -x + y[-1] + 1
    return -a * (b ** -x) + intercept_y + a
...
plt.plot(new_x, new_y,
         label=f'fit: y =  {-a:.2f} * (({b: .3f} * γ)^-x - 1) + {intercept_y: .2f}',
         lw=1,
         color=color_list[0],
         clip_on=False,
         linestyle="dotted")
