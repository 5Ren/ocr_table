...
for i in range(row_numbers - 1):
    ...
    # R2求める
    residuals = current_y_data - nonlinear_fit(x, a, b, c)
    ...
    # 新しいx軸の範囲に対応するy値を予測
    new_y = nonlinear_fit(new_x, a, b, c)
    ...
    plt.plot(new_x, new_y,
             label=f'fit: y = {b:.2f} x ^ {a:.2f} + {c:.2f}' + r' $\mathrm{R}^2$: ' + f'{r_squared: .3f}',
             lw=1,
             color=color_list[i],
             clip_on=False,
             linestyle="dotted")

plt.xlabel(r'$f_{2}$/ $f_{1}$ ratio')
plt.ylabel(r'Bounce height of first time, ${H}_\mathrm{first}$ (mm)')
plt.grid(False)
plt.xlim(0, 10)
plt.ylim(0, 10)
plt.legend()
